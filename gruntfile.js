...
module.exports = function(grunt) {
  ...
  /*
    Helper methods.
    The methods below use the 'moduleFileOrder' Array in
  ...
  /*
    ...
    'moduleFileOrder' Array in 'grunt.conf.js'.
   ...
   */
  var findModuleFilesIn, glob, gruntConfig, gruntTasks, processBuildScripts;
  findModuleFilesIn = function(modulePath) {
    var globPath, j, len, output, ref;
    output = [];
    i;
    ref = gruntConfig.moduleFileOrder;
    for (j = 0, len = ref.length; j < len; j++) {
      globPath = ref[j];
      output.push(modulePath + gruntConfig.moduleFileOrder[globPath]);
    }
    return output;
  };
   ...
   */
  processBuildScripts = function(content) {
    var buildScripts, customModules;
    buildScripts = [];
    globFiles;
    customModules = gruntConfig.build.vendor_js.concat(findModuleFilesIn('./src/app/')).concat(findModuleFilesIn('./src/common/'));
    customModules.forEach(function(module) {
      var globFiles;
      globFiles = glob.sync(module);
      if (globFiles.length > 0) {
        return buildScripts = buildScripts.concat(globFiles);
      }
    });
  ...
  gruntTasks = {
    ...
    copy: {
      build_app_js: {
        src: [findModuleFilesIn('./src/app/'), findModuleFilesIn('./src/common/')],
        dest: '<%= build_dir %>'
      },
      build_app_data: {
        src: ['./src/app/**/*.json'],
      ...
      compile_index: {
        ...
        expand: true,
        options: {
          process: processCompileIndex(function(content) {
            return grunt.template.process(content);
          })
        }
      },
    ...
    clean: {
      ...
      clean_samples: ['./src/app/about/', './src/app/home/', './src/app/profile-feature/', './src/app/shared/profile-model']
    },
    ...
    cssmin: {
      build_css: {
        src: ['<%= build.vendor_css %>', './src/assets/**/*.css', '<%= build_dir %>/src/assets/css/app.custom.css'],
    ...
    uglify: {
      compile_module_js: {
        ...
        src: ['<%= compile.vendor_js %>', findModuleFilesIn('./build/src/app/'), findModuleFilesIn('./build/src/common/')],
    ...
    concat: {
      compile_js: {
        src: ['<%= compile.vendor_min_js %>', '<%= compile_dir %>/src/app/modules.min.js', '<%= compile_dir %>/shared-views/views.min.js'],
        dest: '<%= compile_dir %>/src/app/app.min.js',
        options: {
          banner: '<%= meta.banner %>'
        }
   ...
   */
  grunt.initConfig(grunt.util._.extend(gruntTasks, gruntConfig));
  ...
  grunt.registerTask('build', ['clean:build_clean', 'jshint', 'copy:build_app_js', 'copy:build_app_data', 'copy:build_vendor_js', 'copy:build_views', 'copy:build_shared_views', 'copy:build_fonts', 'copy:build_index', 'copy:build_unit', 'copy:build_protractor', 'copy:build_karma', 'copy:build_assets', 'less:build_less', 'cssmin:build_css', 'clean:build_css_clean', 'autoprefixer:build_autoprefix_css']);
  grunt.registerTask('compile', ['jshint', 'build', 'copy:compile_views', 'html2js:compile_shared_views', 'uglify:compile_module_js', 'concat:compile_js', 'uglify:compile_soft_uglify', 'clean:compile_js_clean', 'clean:compile_shared_views_clean', 'copy:compile_app_data', 'copy:compile_app_assets', 'copy:compile_index']);
